...
const DEFAULT_GAMEMODES = new Map<number, GameModeConfig>([
    [10000, ClassicCTFConfig],
    [10001, FourTeamCTFConfig]
]);
...
export function OnGameModeStarted() {
    ...
    let foundMode = false;
    let currentModeId = 10000;
    let maxModeId = 10001;
    while(!foundMode && currentModeId <= maxModeId){
        ...
        if(gameModeConfigObj){
            ...
            config = DEFAULT_GAMEMODES.get(gameModeConfigId);
            if(config){
                foundMode = true;
                console.log(`Found game mode with id ${gameModeConfigId}`);
            }
        }
